...
job_keywords = ["Java", "Spring", "React", "SQL", "AWS", "Docker", "Microservices", "REST", "Git", "Kubernetes"]
...
def score_resume():
    ...
    education_completed = data.get('education_completed', False)
    ...
    if len(skills) >= 5:
        ...
        suggestions.append("Add more relevant technical skills (at least 5).")

    # Experience check
    if experience_years >= 2:
        score += 25
    else:
        suggestions.append("Include more relevant work or internship experience.")

    # Education check
    if education_completed:
        score += 15
    else:
        suggestions.append("Mention your educational qualifications clearly.")

    # Certifications check
    if len(certifications) > 0:
        score += 10
    else:
        suggestions.append("Add relevant certifications (e.g., AWS, Google, etc.).")

    # Keyword matching
    matched_keywords = sum(1 for kw in job_keywords if kw.lower() in content)
    if matched_keywords >= 5:
        score += 30
    elif matched_keywords > 0:
        partial_score = int((matched_keywords / 5) * 30)
        score += partial_score
        suggestions.append(f"Include more job-specific keywords. Currently matched {matched_keywords}.")
    else:
        suggestions.append("Use more job-specific keywords from the job description.")
...
# API Endpoint: Upload resume PDF and get parsed data + predictions
...
def upload_resume():
    ...
    if file:
        ...
        return jsonify({
            ...
            'skills': skills,
            'education': education,
            'experience_years': 3,  # Dummy data, extract if possible
            'education_completed': True if education else False,
            'certifications': ['AWS Certified Developer'],  # Dummy, extract if needed
            'content': text  # important for keyword scoring 
        })
